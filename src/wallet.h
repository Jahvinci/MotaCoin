...
// StakeForCharityEntry struct is now defined in walletdb.h to avoid circular dependency
// Forward declare here since walletdb.h is included
...
public:
	...
	//StakeForCharity - new direct output implementation (replaces legacy single-address version)
	bool fStakeForCharity;
	std::vector<StakeForCharityEntry> vStakeForCharity;  // Up to 5 entries supported
	...
	// Legacy StakeForCharity() removed - charity is now embedded directly in CreateCoinStake()
	bool MultiSend();
