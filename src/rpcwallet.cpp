...
Value stakeforcharity(const Array &params, bool fHelp)
{
	...
	// Remove command: stakeforcharity remove <address>
	...
	// Add command: stakeforcharity add <address> <percent>
	if(params.size() == 3 && params[0].get_str() == "add")
	{
		...
		// Check if already at max (5 addresses)
	...
	// Legacy format: stakeforcharity <address> <percent> (for backward compatibility)
	if(params.size() == 2)
	{
		if (pwalletMain->IsLocked())
			throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED, "Error: Please enter the wallet passphrase with walletpassphrase first.");
		...
		// Legacy format: clear existing and add single entry
		LOCK(pwalletMain->cs_wallet);
		std::vector<StakeForCharityEntry> vCharityTemp = pwalletMain->vStakeForCharity;
		pwalletMain->vStakeForCharity.clear();
...
Value repairwallet(const Array& params, bool fHelp)
{
    ...
    // Temporarily suppress SQLite FTS5 POSIX collation warning messages
    // These harmless informational messages can appear during wallet operations
    // when SQLite FTS5 extension is loaded. They don't affect functionality.
#ifndef WIN32
    fflush(stderr);
    ...
    if (stderr_backup_fd >= 0) {
        if (freopen("/dev/null", "w", stderr) == NULL) {
            // Failed to redirect stderr, but continue anyway
        }
    }
#endif
    ...
    if (stderr_backup_fd >= 0) {
        ...
        setvbuf(stderr, NULL, _IONBF, 0); // Unbuffered like original stderr
    }
#endif
    int64_t nConflictedBalance = pwalletMain->GetConflictedBalance();
    Object result;
