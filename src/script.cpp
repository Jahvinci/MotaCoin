...
bool Solver(const CScript& scriptPubKey, txnouttype& typeRet, vector<vector<unsigned char> >& vSolutionsRet)
{
    ...
    // OP_RETURN data carrier (for bridge and other metadata)
    // Format: OP_RETURN [data] (max 80 bytes of data)
    if (scriptPubKey.size() >= 1 && scriptPubKey[0] == OP_RETURN)
    {
        // Security: Enforce maximum size limit (OP_RETURN + up to 80 bytes data)
        if (scriptPubKey.size() <= 83)
        {
            typeRet = TX_NULL_DATA;
            return true;
        }
        // Oversized OP_RETURN is non-standard
