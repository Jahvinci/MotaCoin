...
unsigned int nStakeMaxAge = 25 * 24 * 60 * 60; // 25 days
...
bool CTransaction::IsStandard() const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout) {
        ...
        // Check for OP_RETURN (TX_NULL_DATA) outputs
        if (txout.scriptPubKey.size() >= 1 && txout.scriptPubKey[0] == OP_RETURN)
        {
            nDataOut++;
            // Security: Only allow one OP_RETURN output per transaction
            if (nDataOut > 1)
                return false;
            // Security: OP_RETURN must have zero value (provably unspendable)
            if (txout.nValue != 0)
                return false;
        }
        else if (txout.nValue == 0)
        {
            // Non-OP_RETURN outputs cannot have zero value
            return false;
        }
...
int64_t CTransaction::GetMinFee(unsigned int nBlockSize, enum GetMinFee_mode mode, unsigned int nBytes, int nHeight) const
{
    // Use nBestHeight if nHeight not provided (for mempool/wallet operations)
    if (nHeight < 0)
        nHeight = nBestHeight;
    
    // Base fee is either MIN_TX_FEE or MIN_RELAY_TX_FEE (height-aware)
    int64_t nMinTxFee = GetMinTxFeeForHeight(nHeight);
    int64_t nBaseFee = (mode == GMF_RELAY) ? nMinTxFee : nMinTxFee;

    unsigned int nNewBlockSize = nBlockSize + nBytes;
    int64_t nMinFee = (1 + (int64_t)nBytes / 1000) * nBaseFee;

    // To limit dust spam, require minimum fee if any output is less than 0.01
    if (nMinFee < nBaseFee)
    {
        BOOST_FOREACH(const CTxOut& txout, vout)
    ...
    if (nBlockSize != 1 && nNewBlockSize >= MAX_BLOCK_SIZE_GEN/2)
    {
        if (nNewBlockSize >= MAX_BLOCK_SIZE_GEN)
            return GetMaxMoneyForHeight(nHeight);
        nMinFee *= MAX_BLOCK_SIZE_GEN / (MAX_BLOCK_SIZE_GEN - nNewBlockSize);
    }

    if (!MoneyRange(nMinFee, nHeight))
        nMinFee = GetMaxMoneyForHeight(nHeight);
    return nMinFee;
}
                        ...
                        bool* pfMissingInputs)
{
    ...
    if (fCheckInputs)
    {
        ...
        // Don't accept it if it can't get into a block (use current best height for fee validation)
        int64_t txMinFee = tx.GetMinFee(1000, GMF_RELAY, nSize, nBestHeight);
        if (nFees < txMinFee)
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    else if(nHeight < 61742)
    {
        nSubsidy = 75 * COIN;
    }
    else if(nHeight < 123484)
    {
        nSubsidy = 66 * COIN;
    }
    else if(nHeight <= 185226)
    {
        nSubsidy = 55 * COIN;
    }
    else if(nHeight < 246968)
    {
        nSubsidy = 50 * COIN;
    }
    else if(nHeight < 308710)
    {
        nSubsidy = 47 * COIN;
    }
    else if(nHeight <= 370452)
    {
        nSubsidy = 44 * COIN;
    }
    else if(nHeight < 432194)
    {
        nSubsidy = 41 * COIN;
    }
    else if(nHeight < 493936)
    {
        nSubsidy = 39 * COIN;
    }
    else if(nHeight <= 555678)
    {
        nSubsidy = 37 * COIN;
    }
    else if(nHeight < 617420)
    {
        nSubsidy = 35 * COIN;
    }
    else if(nHeight < 679162)
    {
        nSubsidy = 33 * COIN;
    }
    else if(nHeight < 740904)
    {
        nSubsidy = 32 * COIN;
    }
    else if(nHeight < 770420)
    {
        nSubsidy = 31 * COIN;
    }
    else if(nHeight < 802646)
    {
        nSubsidy = 3.1 * COIN;
    }
    else if(nHeight < 864388)
    {
        nSubsidy = 2.9 * COIN;
    }
    else if(nHeight < 926130)
    {
        nSubsidy = 2.6 * COIN;
    }
    else if(nHeight < 987872)
    {
        nSubsidy = 2.3 * COIN;
    }
    else if(nHeight < 1049614)
    {
        nSubsidy = 2 * COIN;
    }
    else if(nHeight < 1111356)
    {
        nSubsidy = 1.7 * COIN;
    }
	else if(nHeight < 1173098)
    {
        nSubsidy = 1.4 * COIN;
    }
    else if(nHeight < 1234526)
    {
        nSubsidy = 1 * COIN;
    }
...
{
	// Premine at PREMINE_BLOCK - 25 million MOTA for chain rollback recovery (15M recovery + 10M lost funds)
	if (nHeight == PREMINE_BLOCK)
		return 25000000 * COIN + nFees;

	//calculate coin reward before super block
	// CRITICAL: MAX_MINT_PROOF_OF_STAKE was changed from 12% to 4.20% in commit e203c61 (Jan 2, 2026)
	// Historical blocks created before this change used 12% rate and must be validated with that rate
	// Use old rate (12%) for blocks up to and including SNAPBACK_PROTOCOL_BLOCK, new rate (4.20%) for blocks after
	static const int64_t OLD_MAX_MINT_PROOF_OF_STAKE = 12 * CENT; // 12% per year (historical)
	static const int64_t NEW_MAX_MINT_PROOF_OF_STAKE = 42 * CENT / 10; // 4.20% per year (current)

	// Use old rate (12%) for blocks up to and including block 1159012
	// New rate (4.20%) applies starting at block 1159013
	// Note: Commit e203c61 changed MAX_MINT_PROOF_OF_STAKE on Jan 2, 2026
	// The 4.20% code change came with the premine code (block 1159015)
	// User confirmed: was stuck at block 1159012, so cutoff is 1159012
	// Use 12% for blocks <= 1159012, 4.20% for blocks >= 1159013
	int64_t nRewardCoinYear = (nHeight <= 1159012) ? OLD_MAX_MINT_PROOF_OF_STAKE : NEW_MAX_MINT_PROOF_OF_STAKE;

    // Handle both overflow (large stakes) and truncation (small stakes)
    // Threshold: 2 trillion coin-age is safe for multiply-first without overflow
    // NOTE: For historical blocks, use old calculation order (divide by 365 first, then COIN)
    // to match how blocks were originally created. The order matters with integer division.
    int64_t nSubsidy;
    if (nCoinAge > 2000000000000LL) {
        // Large stakes: divide first to prevent overflow
        nSubsidy = (nCoinAge / COIN) * nRewardCoinYear / 365;
    } else {
        // Small/medium stakes: use old calculation order for historical compatibility
        // Old order: nCoinAge * nRewardCoinYear / 365 / COIN (divide by 365 first)
        // New order: nCoinAge * nRewardCoinYear / COIN / 365 (divide by COIN first)
        // Use old order to match how historical blocks were created
...
bool CTransaction::DisconnectInputs(CTxDB& txdb)
{
    ...
    if (!IsCoinBase())
    {
        BOOST_FOREACH(const CTxIn& txin, vin)
        {
            ...
            {
                // Transaction index missing - this can happen if the transaction was never indexed
                // or was already removed. Log warning but don't fail for reorganization
                ...
                // For reorganization, we can skip missing transaction indices
                // The transaction may have been in a reorganized-away chain
    ...
    // Note: EraseTxIndex may fail silently if the transaction doesn't exist in the index
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    // Get block height for height-aware fee and money validation (old values before PREMINE_BLOCK, new at/after)
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Check for negative or overflow input values (height-aware validation)
            nValueIn += txPrev.vout[prevout.n].nValue;
            if (!MoneyRange(txPrev.vout[prevout.n].nValue, nBlockHeight) || !MoneyRange(nValueIn, nBlockHeight))
        ...
        if (!IsCoinStake())
        {
            ...
            // enforce transaction fees for every block (height-aware: old fee before PREMINE_BLOCK, new fee at/after)
            // nBlockHeight already set at function start
            int64_t nMinFee = GetMinFee(1, GMF_BLOCK, 0, nBlockHeight);
            if (nTxFee < nMinFee)
            ...
            nFees += nTxFee;
            if (!MoneyRange(nFees, nBlockHeight))
...
bool static Reorganize(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    BOOST_FOREACH(CBlockIndex* pindex, vDisconnect)
    {
        ...
        // MotaCoin: remove stake from setStakeSeen when disconnecting blocks
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    // SANITY CHECK: Comment out printf that was added for debugging
    // This printf is called for EVERY block and might be causing crashes during wallet loading
    // if (fDebug && GetBoolArg("-printblockcheck"))
    ...
    if (IsProofOfStake())
    {
        ...
        // Safety check: ensure we have at least 2 transactions (coinbase + coinstake) before accessing vtx[1]
        if (vtx.size() < 2)
...
bool CBlock::AcceptBlock()
{
    ...
    // Enforce block version AFTER SNAPBACK_PROTOCOL_BLOCK (enforcement begins at block 1159001)
    if (nHeight > SNAPBACK_PROTOCOL_BLOCK && nHeight < PREMINE_BLOCK && nVersion < CBlockHeader::POST_SNAPBACK_VERSION)
        return DoS(100, error("AcceptBlock() : block version %d is below required version %d at height %d (requires version %d after block %d)",
                              nVersion, CBlockHeader::POST_SNAPBACK_VERSION, nHeight, CBlockHeader::POST_SNAPBACK_VERSION, SNAPBACK_PROTOCOL_BLOCK));
    
    // Enforce block version at/after PREMINE_BLOCK (enforcement begins at block 1159015)
    if (nHeight >= PREMINE_BLOCK && nVersion < CBlockHeader::PREMINE_VERSION)
        return DoS(100, error("AcceptBlock() : block version %d is below required version %d at height %d (requires version %d at/after PREMINE_BLOCK %d)",
                              nVersion, CBlockHeader::PREMINE_VERSION, nHeight, CBlockHeader::PREMINE_VERSION, PREMINE_BLOCK));
	...
	try
	{
		...
		//
		if(nHeight > 10000 &&
			!IsProofOfStake() &&  // Skip PoS blocks - they are validated in ConnectBlock with GetProofOfStakeReward
			// First Attack
	...
	catch(...)
	{
		...
		//
		if(nHeight > 701720)    // stop accepting these after bootstrap
		{
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Duplicate stake allowed when:
    // - There is an orphan child block waiting for this block
    // - The block's parent exists but is NOT on the best chain (allows forks to compete)
    // - Pending sync checkpoint wants this block
    // - During initial block download (bootstrap loading)
    if (pblock->IsProofOfStake() && setStakeSeen.count(pblock->GetProofOfStake()))
    {
        ...
        {
            // During initial block download, allow fork blocks to compete
            // This is necessary when loading bootstrap files with fork blocks
            fAllowDuplicate = true;
        }
        else if (mapBlockIndex.count(pblock->hashPrevBlock))
        {
            // Check if parent is NOT on best chain (it's a fork)
            // If parent is not the best chain tip, this block is extending a fork
            if (pblock->hashPrevBlock != hashBestChain)
            {
                // Allow fork blocks to compete (they may reorganize the chain)
                fAllowDuplicate = true;
            }
        }
	...
	// Legacy StakeForCharity removed - charity is now embedded directly in stake transactions
	// No post-block processing needed for charity anymore
...
bool CBlock::CheckBlockSignature() const
{
    ...
    // Safety check: ensure we have at least 2 transactions (coinbase + coinstake)
    if (vtx.size() < 2)
    {
        if (fDebug && GetBoolArg("-printcoinstake"))
    ...
    // Safety check: ensure coinstake transaction (vtx[1]) exists and has outputs
    // This must be checked before accessing vtx[1].vout to avoid crashes
    if (vtx[1].vout.empty())
    {
        if (fDebug && GetBoolArg("-printcoinstake"))
    ...
    // Safety check: ensure we have at least 2 outputs (marker + staker output)
    // Old blocks might have only 1 output (marker), which would be invalid for PoS
    ...
    txnouttype whichType = TX_NONSTANDARD;  // Initialize to avoid uninitialized variable

    // Find the staker output (first TX_PUBKEY output after marker)
    // vout[0] is the marker (empty), then charity outputs (TX_PUBKEYHASH), then staker output(s) (TX_PUBKEY)
    // We need to find the first TX_PUBKEY output, which is the staker's output
    bool bFoundStakerOutput = false;

    // Search through outputs starting from index 1 (skip marker at index 0)
    ...
    // Fallback to vout[1] for backward compatibility with traditional blocks
    // This should only happen for old blocks where vout[1] is the staker output (TX_PUBKEY)
    // Note: We already checked vtx[1].vout.size() >= 2 above, so vtx[1].vout[1] is safe to access
    if (!bFoundStakerOutput)
    {
        ...
        // Verify fallback output is actually TX_PUBKEY (staker output)
        // If it's not TX_PUBKEY (e.g., TX_PUBKEYHASH for charity), signature check will fail below
    ...
    // Verify signature using the found staker output
    if (whichType == TX_PUBKEY)
    {
        // Safety check: ensure we have a solution before accessing it
        if (vSolutions.empty())
        {
            if (fDebug && GetBoolArg("-printcoinstake"))
    ...
    // If whichType is not TX_PUBKEY, signature check fails
    if (fDebug && GetBoolArg("-printcoinstake"))
...
bool LoadExternalBlockFile(FILE* fileIn)
{
    ...
    int nLoaded = 0;      // Blocks successfully added (new blocks)
    int nProcessed = 0;   // Total blocks processed from file (including already-existing blocks)
        ...
        try {
            ...
            while (nPos != (unsigned int)-1 && blkdat.good() && !fRequestShutdown)
            {
                ...
                if (nSize > 0 && nSize <= MAX_BLOCK_SIZE)
                {
                    CBlock block;
                    blkdat >> block;
                    nProcessed++;  // Count all blocks processed from file
                    if (ProcessBlock(NULL,&block))
                    {
                        nLoaded++;  // Count only new blocks successfully added
                    }
                    nPos += 4 + nSize;
                    
                    // Report progress: first 1000 blocks more frequently, then every 1000 blocks or every 1% of file
                    ...
                    // Report more frequently for first 1000 blocks processed, then every 1000 blocks
                    if (nProcessed <= 1000) {
                    ...
                    if (bShouldReport)
                    {
                        ...
                        if (nFileSize > 0) {
                            // Show blocks processed and file progress percentage
    ...
    // MotaCoin: After loading all blocks from bootstrap file, try to connect any orphan blocks
    // This handles cases where blocks were out of order or on different forks
    if (mapOrphanBlocks.size() > 0)
    {
        ...
        // Keep trying to connect orphans until no more can be connected
        bool fProgress = true;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
	...
	vRecv >> addrMe;
        ...
        {
            // Protocol version MIN_PROTO_VERSION (1.6.0) is required
            // Earlier versions are no longer supported and will be disconnected
	...
	if (pfrom -> nVersion == PROTOCOL_VERSION_MOBILE) {
        ...
        // Enforce new protocol version AFTER SNAPBACK_PROTOCOL_BLOCK (enforcement begins at block 1159001)
        // Only enforce when OUR chain is past 1159000 - allow old protocol versions when syncing up to that block
        // This allows syncing from old network up to block 1159000, then requires new protocol after
        if (nBestHeight > SNAPBACK_PROTOCOL_BLOCK)
        {
            if (pfrom->nVersion < MIN_PROTO_VERSION_AFTER_SNAPBACK && pfrom->nVersion != PROTOCOL_VERSION_MOBILE)
            {
                printf("partner %s at height %d using obsolete version %i (requires %i after block %d); disconnecting\n",
                       pfrom->addr.ToString().c_str(), pfrom->nStartingHeight, pfrom->nVersion,
                       MIN_PROTO_VERSION_AFTER_SNAPBACK, SNAPBACK_PROTOCOL_BLOCK);
                pfrom->fDisconnect = true;
                return false;
            }
        }
