...
int64_t CWallet::GetUnconfirmedBalance() const
{
    int64_t nTotal = 0;
    {
        ...
        {
            const CWalletTx* pcoin = &(*it).second;
            // Exclude conflicted transactions (depth < 0) - they never happened
            // Only include truly unconfirmed transactions (depth >= 0 but not confirmed)
...
int64_t CWallet::GetConflictedBalance() const
{
    int64_t nTotal = 0;
    {
        ...
        {
            const CWalletTx* pcoin = &(*it).second;
            // Count conflicted transactions (depth < 0) - these never happened but are still in wallet
            if (!(pcoin->IsCoinBase() || pcoin->IsCoinStake()) && pcoin->GetDepthInMainChain() < 0)
            {
                // For conflicted transactions, show net effect (credit - debit)
                // Negative means we "lost" money (outgoing), positive means we "gained" (incoming)
...
bool CWallet::MultiSend()
{
...

    {
		...
		// Check if we already processed MultiSend at this height
...
// Legacy StakeForCharity() function removed
// Charity outputs are now embedded directly in CreateCoinStake() when stake is created
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                // Check that enough fee is included (use current best height for fee validation)
                int64_t nPayFee = nTransactionFee * (1 + (int64_t)nBytes / 1000);
                int64_t nMinFee = wtxNew.GetMinFee(1, GMF_SEND, nBytes, nBestHeight);
...
bool CWallet::GetStakeWeight2(const CKeyStore& keystore, uint64_t& nMinWeight, uint64_t& nMaxWeight, uint64_t& nWeight, uint64_t& nHoursToMaturity, uint64_t& nAmount)
{
	...
	uint64_t nPrevAge = 0;
	uint64_t nStakeAge = nStakeMinAge; // 60 * 60 * 24 * 2;
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        CTxIndex txindex;
       {
		...
		if (nCurrentAge < nStakeAge) // (60*60*24*2)) // if the age is less than 2 days, report weight as 0 because the stake modifier won't allow for stake yet
			bnCoinDayWeight = 0;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    // StakeForCharity: Add charity outputs if enabled
    // Track original output count to detect split stake (before charity outputs are added)
    unsigned int nOriginalOutputCount = txNew.vout.size();  // Will be 2 (single) or 3 (split) before charity
    int64_t nTotalCharity = 0;
    int nCharityOutputCount = 0;
    if (fStakeForCharity && !vStakeForCharity.empty())
    {
        // Calculate charity amounts from reward (not total credit which includes original coins)
        // nReward is the pure stake reward, which is what we want to split for charity

        for (unsigned int i = 0; i < vStakeForCharity.size() && i < 5; i++)  // Max 5 addresses
        {
            ...
            if (nCharityAmount > 0 && nCharityAmount >= MIN_RELAY_TX_FEE)
            {
                ...
                if (charityAddress.IsValid())
                {
                    ...
                    // Insert charity outputs after marker (vout[0]) but before staker outputs
                    // We'll insert at position 1 + nCharityOutputCount
        ...
        // Subtract charity from credit (staker gets remaining)
        // Only subtract if we actually added charity outputs
        // Safety: ensure we don't subtract more than the reward
        if (nTotalCharity > 0)
        {
            if (nTotalCharity > nReward)
            {
                ...
                nTotalCharity = nReward;  // Cap at reward amount
            }
            nCredit -= nTotalCharity;
        }
    }

    // Safety check: ensure nCredit is positive
    if (nCredit <= 0)
    {
        if (fDebug && GetBoolArg("-printcoinstake"))
    ...
    // Set output amount for staker outputs
    // Account for charity outputs: vout[0] = marker, vout[1 to 1+nCharityOutputCount-1] = charity, then staker outputs
    int nFirstStakerOutput = 1 + nCharityOutputCount;  // First staker output index
    ...
    // Original check was if (txNew.vout.size() == 3) for split stake
    // Now: nOriginalOutputCount == 3 means split stake (marker + 2 staker outputs)
    if (nOriginalOutputCount == 3)  // Split stake: marker + 2 staker outputs (before charity)
    {
        // Safety check: ensure we have a second staker output
    ...
    else  // Single stake: marker + 1 staker output (before charity)
    {
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, int& nOrphansFound, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        // Check for conflicted receive transactions (not from us, conflicted, not in blockchain)
        // A receive transaction is one where GetDebit() == 0 (we didn't send it)
        if (!(pcoin->IsCoinBase() || pcoin->IsCoinStake()) && !fTxInBlockchain && pcoin->GetDepthInMainChain() < 0 && pcoin->GetDebit() == 0)
        {
            // Check if this transaction has any received amounts (outputs to us)
            ...
            if (fHasReceived)
            {
                ...
                if (!fCheckOnly)
                {
                    printf("FixSpentCoins removing conflicted receive tx %s (amount: %s MOTA)\n",
                ...
                {
                    printf("FixSpentCoins found conflicted receive tx %s (amount: %s MOTA), repair not attempted\n",
                           hash.ToString().c_str(), FormatMoney(nReceivedAmount).c_str());
                }
                continue;
            }
        ...
        // Check for conflicted outgoing transactions (from us, conflicted, not in blockchain)
        // An outgoing transaction is one where GetDebit() > 0 (we sent it)
        if (!(pcoin->IsCoinBase() || pcoin->IsCoinStake()) && !fTxInBlockchain && pcoin->GetDepthInMainChain() < 0 && pcoin->GetDebit() > 0)
        {
            // Before removing, restore spent state of inputs if they're unspent in blockchain
            ...
            BOOST_FOREACH(const CTxIn& txin, pcoin->vin)
            {
                ...
                if (mi != mapWallet.end())
                {
                    ...
                    {
                        // Check if this input is actually unspent in blockchain
                        ...
                        {
                            // Input is unspent in blockchain, restore it in wallet
            ...
            // Count the transaction for reporting
            int64_t nCredit = pcoin->GetCredit();
            int64_t nDebit = pcoin->GetDebit();
            int64_t nNet = nCredit - nDebit;
            nMismatchFound++;
            // Use absolute value for reporting (negative net means we lost money, positive means we got change)
            nBalanceInQuestion += (nNet < 0 ? -nNet : nNet);
            if (!fCheckOnly)
            {
                printf("FixSpentCoins removing conflicted outgoing tx %s (net: %s MOTA, debit: %s MOTA, credit: %s MOTA)\n",
            ...
            {
                printf("FixSpentCoins found conflicted outgoing tx %s (net: %s MOTA), repair not attempted\n",
