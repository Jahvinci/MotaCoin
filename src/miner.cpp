...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake, int64_t* pFees)
{
    ...
    int64_t nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // Transaction fee (use next block height for validation)
            int nHeight = pindexPrev->nHeight + 1;
            int64_t nMinFee = tx.GetMinFee(nBlockSize, GMF_BLOCK, 0, nHeight);
        ...
        if (!fProofOfStake)
        {
            pblock->vtx[0].vout[0].nValue = GetProofOfWorkReward(pindexBest->nHeight, nFees) - devCoin;
        ...
        // Set block version based on height:
        // - Version 6: blocks <= 1159000 (PRE_SNAPBACK_VERSION)
        // - Version 7: blocks 1159001-1159014 (POST_SNAPBACK_VERSION)
        // - Version 8: blocks >= 1159015 (PREMINE_VERSION/CURRENT_VERSION)
        int nHeight = pindexPrev->nHeight + 1;
        if (nHeight >= PREMINE_BLOCK)
            pblock->nVersion = CBlockHeader::PREMINE_VERSION;
        else if (nHeight > SNAPBACK_PROTOCOL_BLOCK)
            pblock->nVersion = CBlockHeader::POST_SNAPBACK_VERSION;
        else
            pblock->nVersion = CBlockHeader::PRE_SNAPBACK_VERSION;
    }
