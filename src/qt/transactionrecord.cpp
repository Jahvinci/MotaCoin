...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    if (wtx.IsCoinStake())
    {
        // For CoinStake, calculate actual credit from outputs that belong to this wallet
        // (not GetValueOut() which includes all outputs including charity donations)
		...
		// Find the address of the output that belongs to this wallet
		// Prioritize the largest output (staker's own reward) over charity outputs
		// This handles cases where multiple charity addresses are in the same wallet
		...
		BOOST_FOREACH(const CTxOut& txout, wtx.vout)
		{
			if(wallet->IsMine(txout))
			{
				// Prefer the largest output (staker's reward is typically much larger than charity donations)
