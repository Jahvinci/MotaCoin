...
void ScriptPubKeyToJSON(const CScript& scriptPubKey, Object& out, bool fIncludeHex)
{
    ...
    // Handle OP_RETURN (TX_NULL_DATA) specially since it has no destinations
    if (scriptPubKey.size() >= 1 && scriptPubKey[0] == OP_RETURN)
    {
        if (scriptPubKey.size() <= 83)  // Max 80 bytes data + 3 bytes overhead
        {
...
void TxToJSON(const CTransaction& tx, const uint256 hashBlock, Object& entry)
{
    ...
    // For unconfirmed transactions, use transaction time
    // For confirmed transactions, block time will be set later (line 112)
    if (hashBlock == 0)
