...
make -C depends --jobs="$JOBS" HOST="$HOST" \
                                   ...
                                   qt_config_opts_i686_linux='-platform linux-g++ -xplatform motacoin-linux-g++' \
                                   qt_config_opts_x86_64_linux='-platform linux-g++ -xplatform motacoin-linux-g++' \
...
# T    sed -i.old 's/-lstdc++ //g' config.status libtool src/univalue/config.status src/univalue/libtool
    ...
    # Setup the directory where our MotaCoin build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built MotaCoin to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        case "$HOST" in
            *mingw*)
# come back to this, i think this is for the concensus dll
